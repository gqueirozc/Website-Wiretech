...
export default function PaginaFinalizarCompra() {
  ...
  carrinho.valor_produto = round2(
    carrinho.carrinhoItems.reduce(
      (a, c) => a + c.quantidade * c.valor_produto,
      0
    )
  );
  if (shipping.pais === "Brasil") {
    carrinho.preco_total = carrinho.valor_produto + 15;
  } else {
    carrinho.preco_total = carrinho.valor_produto + 100;
  }
  ...
  const registrarTransacao = async () => {
    for (let i = 0; i < carrinho.carrinhoItems.length; i++) {
      ...
      let id = carrinho.carrinhoItems[i].id_produto;
      descricao[i] =
            ...
            <Card.Body>
              ...
              <ListGroup variant="flush">
                <ListGroup.Item>
                  <Row>
                    <Col>Items</Col>
                    <Col>R$ {carrinho.valor_produto}</Col>
                  </Row>
                </ListGroup.Item>
                <ListGroup.Item>
                  <Row>
